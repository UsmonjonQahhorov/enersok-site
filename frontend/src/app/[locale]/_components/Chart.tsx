...
export const SponsorDonutChart = ({ data }: SponsorDonutChartProps) => {
	const Config: ChartConfig = data.reduce(
		(acc: Record<string, { color: string; value: number }>, curr) => {
			acc[curr.name] = {
				color: curr.color,
				value: curr.value,
			};
	...
	return (
		<>
			...
			<ChartContainer config={Config} className="h-[350px] md:h-[613px]">
				<PieChart className="flex justify-center [&>svg]:!w-[60%] [&>svg]:md:!w-full [&>div]:hidden [&>div]:md:block">
					...
					{/* On hover show value */}
					<ChartTooltip formatter={(value: string) => `${value}`} />
				</PieChart>
			</ChartContainer>
		</>
	);
