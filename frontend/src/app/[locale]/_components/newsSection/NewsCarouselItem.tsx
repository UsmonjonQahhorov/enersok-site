	...
	return (
		<div className="xl:flex gap-8 md:gap-10 h-full">
			<article className="mb-[52px] md:mb-10 xl:mb-0">
				<Link
					href={RouterConfig.SingleNew(data?.[0]?.slug ?? '')}
					className={cn('flex flex-col items-center text-secondary h-full')}
				>
					<NextImage
					...
					<div className="flex flex-col bg-backgroundImage1 w-full h-full rounded-b-xl px-6 pt-6 md:pt-8">
						<Heading
							as="h4"
							className="text-2xl text-wrap md:text-[32px] overflow-hidden"
						>
							{data?.[0]?.title}
						</Heading>
			...
			<div className="flex flex-col gap-10">
				{data.slice(1).map((item) => (
					<article key={item.image.url}>
						<Link
							href={RouterConfig.SingleNew(item.slug)}
							className={cn('flex gap-6 items-center text-secondary')}
						>
							<NextImage
								src={item.image.url}
							...
							<div className="flex flex-col">
								<Heading
									as="h4"
									className="text-2xl md:text-[28px] text-wrap xl:max-w-[400px]"
								>
