...
// import { BASE_URL, NODE_ENV, TOKEN } from '@/configs/env.config';
// import { checkEnvironment } from '@/utils/env.util';

// import { FetchError } from './exceptions';

// type HttpUrl = RequestInfo | URL;
// type HttpOptions =
//   | (RequestInit & {
...
//     })
//   | undefined;
// type FailedHttpResult<ErrorType> = {
//   ok: false;
//   code: number;
//   data: null;
//   error: ErrorType;
// };
// type SuccessHttpResult<ResponseType> = {
//   ok: true;
//   code: number;
//   data: ResponseType;
//   error: null;
// };
// export type HttpResult<S, E> = SuccessHttpResult<S> | FailedHttpResult<E>;
...
//     // ==== Sending the request ==== //
//     const rawResponse = await fetch(requestUrl, requestOptions);

//     // ==== Parsing the response ==== //
//     const responseCode = rawResponse.status;
//     const responseOk = rawResponse.ok;

//     // ==== Checking if response status code was not between 200-299 ==== //
//     if (responseOk === false) {
//       const clonedResponse = rawResponse.clone();
//       const isValidJson = isJsonResponse(clonedResponse);
...
//     // ==== Parsing the response ==== //
//     const responseData = (await rawResponse.json()) as ResponseType;
...
//     };
//   } catch (error) {
//     const enviroment = checkEnvironment();
//     // ==== Logging the error ==== //
//     if (NODE_ENV === 'development' || enviroment === 'server') {
//       console.error('[HTTP ERROR]', `URL: ${requestUrl}`, `Error: ${error}`);
//     }

//     // ==== Preparing the error ==== //
//     let errorCode = 500;
//     let errorMesssage = 'Произошла неизвестная серверная ошибка';

//     // ==== Checking if the error is a FetchError ==== //
//     if (error instanceof FetchError) {
//       errorCode = error.code;
//       const isJsonMessage = isJsonError(error.message);
//       errorMesssage = isJsonMessage ? `${error.message}` : error.message;
//     }
...
//       code: errorCode,
//       data: null,
//       error: errorMesssage as ErrorType,
//       ok: false,
//     };
//   }
// };
...
// function isJsonError(str: unknown) {
