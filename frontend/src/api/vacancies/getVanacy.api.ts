...
export const getVacancy = async (locale: Locale = 'en', slug: string): Promise<GetVacancyReturnType> => {
     ...
     if (locale === 'uz') {
          vacancy = {
               vacancyName: data.localizations.data.find((item) => item.attributes.locale === 'uz')?.attributes.vacancy_name || '',
               vacancyLocation: data.localizations.data.find((item) => item.attributes.locale === 'uz')?.attributes.vacancy_location || '',
               vacancyClosingDate: data.localizations.data.find((item) => item.attributes.locale === 'uz')?.attributes.vacancy_closing_date || '',
               vacancyDescription: data.localizations.data.find((item) => item.attributes.locale === 'uz')?.attributes.vacancy_description || '',
               vacancyPublishedDate: data.publishedAt,
          }
     }
